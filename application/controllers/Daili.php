...
class Daili extends CI_Controller {
    ...
    public function reveal(){
        $data['users'] = $this->daili_model->getReveal();
        $this->load->view('daili/templates/header');
        $this->load->view('daili/reveal',$data);
        $this->load->view('daili/templates/footer');
    }
    //删除一条消息
    public function deleteReveal(){
        $id = $this->uri->segment(3);
        $this->daili_model->deleteReveal($id);
    }
    //打开修改页面,并根据ID查询要修改的数据
    public function toUpdateReveal(){
		$city_id = 224;
        $id = $this->uri->segment(3);
        $data['user'] = $this->daili_model->findReveal($id);
		...
		$this->load->view('daili/templates/header');
		$this->load->view('daili/updateReveal',$data);
        $this->load->view('daili/templates/footer');
    }
    //修改一条消息
